...
|实践结果分析|百度|讯飞|
|---|---|---|
|效果对比：场景识别||对于场景识别所使用的图像，我们能直观的看到两排偏橙色的教学楼、中间的绿树与远处的山和泛黄的天空，这是对于视觉正常的人所能达到的直观感受，在百度API返回的结果中，场景中存在的四个主要部分都正确的识别出来，但具体描述（keyword）准确率还较差，特别是对于建筑的识别|反观讯飞API的返回结果，并没有识别出建筑，但对于景色的匹配率较高|
